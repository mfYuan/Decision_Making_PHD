...
class WaypointUpdater(object):
    def __init__(self):
        rospy.init_node('waypoint_updater10')

        rospy.Subscriber('/qcar1/rear_pose', PoseStamped, self.pose_cb)
        rospy.Subscriber('/base_waypoints10', Lane, self.waypoints_cb)

        self.final_waypoints_pub = rospy.Publisher(
            'final_waypoints10', Lane, queue_size=1)
        self.final_path_pub = rospy.Publisher(
            'final_path10', Path, queue_size=1)

        self.idx_pub = rospy.Publisher('idx10', PoseStamped, queue_size=1)
