...
class Purepursuit:
	def __init__(self):
		rospy.init_node('pure_pursuit1', log_level=rospy.DEBUG)

		rospy.Subscriber('/qcar1/rear_pose', PoseStamped, self.pose_cb, queue_size = 1)
		rospy.Subscriber('/qcar1/velocity', TwistStamped, self.vel_cb, queue_size = 1)
		rospy.Subscriber('/final_waypoints1', Lane, self.lane_cb, queue_size = 1)

		self.twist_pub = rospy.Publisher('/qcar1/ackermann_cmd', AckermannDriveStamped, queue_size = 1)
	...
	def calculateTwistCommand(self):
		...
		targetSpeed = self.currentWaypoints.waypoints[0].twist.twist.linear.x
	        '''print("targetspeed of qcar1 is", targetSpeed)'''
		'''print(targetSpeed)'''
