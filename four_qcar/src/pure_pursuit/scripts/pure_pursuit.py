...
class Purepursuit:
	def __init__(self):
		...
		rospy.Subscriber('/qcar/rear_pose', PoseStamped, self.pose_cb, queue_size = 1)
		rospy.Subscriber('/qcar/velocity', TwistStamped, self.vel_cb, queue_size = 1)
		rospy.Subscriber('/final_waypoints', Lane, self.lane_cb, queue_size = 1)

		self.twist_pub = rospy.Publisher('/ackermann_cmd', AckermannDriveStamped, queue_size = 1)
