...
class DQN1:
    def __init__(self):
        ...
        self.first_dense = [self.Num_dataSize, 512]
    ...
    def network(self):
        ...
        with graph1.as_default():
            ...
            # Input------sensor
            self.x_sensor = tf.placeholder(tf.float32, shape=[None, 371])
		...
		h_conv3 = tf.nn.relu(self.conv2d(h_conv2, w_conv3, 1) + b_conv3)'''
            ...
            h_fc1_state = tf.matmul(self.x_sensor, w_fc1_1)+b_fc1_1
            h_fc1_action = tf.matmul(self.x_sensor, w_fc1_2)+b_fc1_2
		...
		h_fc1_action_target = tf.matmul(h_concat_target, w_fc1_2_target)+b_fc1_2_target'''
            ...
            h_fc1_state_target = tf.matmul(
                self.x_sensor, w_fc1_1_target)+b_fc1_1_target
            h_fc1_action_target = tf.matmul(
                self.x_sensor, w_fc1_2_target)+b_fc1_2_target
            # h_fc1_state_target = tf.nn.relu(tf.matmul(h_concat_target, w_fc1_1_target)+b_fc1_1_target)
            # h_fc1_action_target = tf.nn.relu(tf.matmul(h_concat_target, w_fc1_2_target)+b_fc1_2_target)
            h_fc2_state_target = tf.matmul(
                h_fc1_state_target,  w_fc2_1_target)+b_fc2_1_target
            ...
            output_target = tf.add(
                h_fc2_state_target, h_fc2_advantage_target)
    ...
    def input_initialization(self, env_info):
        with graph1.as_default():
            state_stack = env_info[0]  # laser info + self state
            state_set = []
            state_set.append(state_stack)
    ...
    def update_information(self):
        ...
        # print(np.array(self.state_stack).shape)
