...
class envmodel1():
    ...
    def accel_new_old(self, desired_v, pid_output, action_qcar1):
        self.accel_current = action_qcar1
        #self.throttle, theta= self.PID(desired_v)
        speed = pid_output
        if speed < 0:
            speed = 0
        self.cmd[0] = speed
        self.cmd[1] =self.calculateTwistCommand()#theta#self.loop()
        
        #print("car 1 cmd {}".format(self.cmd))

        cmd_vel = AckermannDriveStamped()
        #print('Actual Speed:', self.robotstate1[2])
        cmd_vel.drive.speed = self.cmd[0]
        cmd_vel.drive.steering_angle = self.cmd[1]
        #cmd_vel.drive.acceleration = 0

        self.pub.publish(cmd_vel)
        if speed >0:
            print('desired vel: {}, real-vel: {}, vel-error: {}'.format(desired_v, self.robotstate1[2], desired_v - self.robotstate1[2]))
