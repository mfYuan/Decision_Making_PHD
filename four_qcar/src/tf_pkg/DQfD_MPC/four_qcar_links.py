...
from two_qcar_link1 import DQN1
...
class DQN:
    def __init__(self):
        ...
        # Initial parameters
        # ------------------------------
        self.Num_start_training = 10  # 5000
    ...
    def desired_velocity(self, cmd_v, st, car_num):
        if car_num == 1:
            car_v = link1.env.robotstate1[2]
            desired_v = car_v + cmd_v * st
            if desired_v > self.max_speed:
                desired_v = self.max_speed
        '''elif car_num == 2:
            car_v = self.robotstate2[2]
            desired_v = car_v + cmd_v * st
            if desired_v > max_v:
                desired_v = max_v
        elif car_num == 3:
            car_v = self.robotstate3[2]
            desired_v = car_v + cmd_v * st
            if desired_v > max_v:
                desired_v = max_v
        elif car_num == 4:
            car_v = self.robotstate4[2]
            desired_v = car_v + cmd_v * st
            if desired_v > max_v:
                desired_v = max_v
        elif car_num == 5:
            car_v = self.robotstate5[2]
            desired_v = car_v + cmd_v * st
            if desired_v > max_v:
                desired_v = max_v
        elif car_num == 6:
            car_v = self.robotstate6[2]
            desired_v = car_v + cmd_v * st
            if desired_v > max_v:
                desired_v = max_v
        if desired_v < 0:
    ...
    def PID1(self, cmd_v):
        current_time = 2.4*self.elapsed_time()#np.mean(self.time_factors)*self.elapsed_time()
        ...
        if cmd_v != self.previous_action and cmd_v == 0.0:
            self.maintain =link1.env.robotstate1[2]
        if cmd_v ==0.0:
            try:
        ...
        integral = self.integral_val + (delta_v * st) #intergral

        derivative = (delta_v - self.derivative_val) / st #derivative
        ...
        self.integral_val = integral
        self.derivative_val = delta_v
    ...
    def main(self):
        ...
        self.integral_val = 0.0
        self.derivative_val = 0.0
        self.v_r = []
        self.v_m = []
        self.v_e = []
        self.t = []
        while True:
            ...
            desired_v, pid_output = self.PID1(cmd_qcar1)
            link1.accelerate_new(desired_v, pid_output, cmd_qcar1)
            self.v_r.append(desired_v)
            self.v_m.append(link1.env.robotstate1[2])
            self.v_e.append(desired_v - link1.env.robotstate1[2])
            ...
            '''if self.progress == 'Training' or self.progress == 'Stable Training':
                ...
                if self.progress == 'Observing' and self.step % 50 == 0:
                    print('Observing Took', time.time()-start)
                elif self.progress == 'Not Training' and test_count % 50 == 0:
                    print('Not Training took', time.time()-start)
                    test_count += 1'''
            ...
            if self.step_for_newenv == self.MAXSTEPS or terminal_1 == True:
                ...
                plt.clf()
                plt.plot(self.t, self.v_r, 'k', label='Reference Velocity')
                plt.plot(self.t, self.v_m, 'b--', label='Measured Velocity')
                plt.plot(self.t, self.v_e, 'r:', label='Velocity Error')
                plt.pause(0.05)
                plt.ion()
