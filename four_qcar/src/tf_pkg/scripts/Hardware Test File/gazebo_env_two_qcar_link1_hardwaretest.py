...
from geometry_msgs.msg import Quaternion, PoseStamped, TwistStamped, PoseWithCovarianceStamped, Vector3Stamped
...
class envmodel1():
	...
	self.sub1 = rospy.Subscriber('/poseupdate', PoseWithCovarianceStamped, self.poseupdate_callback)
	self.sub2 = rospy.Subscriber('/qcar/velocity', Vector3Stamped, self.velocity_callback)
	...
	self.Waypoints = rospy.Subscriber('/final_waypoints5', Lane, self.lane_cb, queue_size=1)
        ...
        self.pub = rospy.Publisher('/' + self.agentrobot1 + '/ackermann_cmd', AckermannDriveStamped, queue_size=10)
    ...
    def poseupdate_callback(self, data):
	self.poseupdate = data
        # robotstate--->x,y,v,w,yaw,vx,vy
	self.robotstate1[0] = data.pose.pose.position.x
	self.robotstate1[1] = data.pose.pose.position.y
	rpy = self.euler_from_quaternion(data.pose.pose.orientation.x,data.pose.pose.orientation.y,data.pose.pose.orientation.z,data.pose.pose.orientation.w)
	self.robotstate1[4] = rpy[2]

    def velocity_callback(self, data):
	self.velocity = data
	v = math.sqrt(vector.x**2 + vector.y**2)
	self.robotstate1[2] = v
	self.robotstate1[5] = vector.x
	self.robotstate1[6] = vector.y
...
'''    def gazebo_states_callback(self, data):
	...
	reward = reward - 0.005
        ...
        if math.sqrt((self.robotstate1[0] - self.robotstate2[0])**2 + (self.robotstate1[1] - self.robotstate2[1])**2) < 0.5:
            reward = reward - 5
            print("[Link1] Vehicle Collision: Lose 5 reward.")
	...
	self.Waypoints = rospy.Subscriber('/final_waypoints' + path, Lane, self.lane_cb, queue_size=1)
        self.path = path
