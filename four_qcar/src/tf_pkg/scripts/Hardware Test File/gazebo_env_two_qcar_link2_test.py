...
class envmodel2():
	...
	self.agentrobot2 = 'qcar2'
        ...
        # subscribers
        self.subimage = rospy.Subscriber('/' + self.agentrobot2 + '/csi_front/image_raw', Image, self.image_callback)
        self.subLaser = rospy.Subscriber('/' + self.agentrobot2 + '/lidar', LaserScan, self.laser_states_callback)
	...
	#self.Waypoints = rospy.Subscriber('/final_waypoints5', Lane, self.lane_cb, queue_size=1)
        ...
        self.pub = rospy.Publisher('/' + self.agentrobot2 + '/ackermann_cmd', AckermannDriveStamped, queue_size=10)
    ...
    def get_env(self):
        ...
        # ----------------------------------------
        dx = -(self.robotstate2[0]-self.robotstate2[0]) #######
        dy = -(self.robotstate2[1]-self.robotstate2[1])
        xp = dx*math.cos(self.robotstate2[4]) + dy*math.sin(self.robotstate2[4])
        yp = -dx*math.sin(self.robotstate2[4]) + dy*math.cos(self.robotstate2[4])
	...
	self.other_side = False
        ...
        elif (self.robotstate2[0] < -0.94 and self.robotstate2[1] > 0.18 and self.path=='6'):
	    self.other_side = True
            self.done_list = True  # 终止 (terminated due to episode completed)
	    #print("Reached other side!")
        ...
        elif (self.robotstate2[0] > 0.94 and self.robotstate2[1] < -0.18 and self.path=='8'):
	    self.other_side = True
            self.done_list = True  # 终止 (terminated due to episode completed)
	    #print("Reached other side!")
