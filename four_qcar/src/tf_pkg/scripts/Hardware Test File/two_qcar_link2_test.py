...
keras.backend.tensorflow_backend.set_session(tf.Session(graph=graph2, config=config2))
...
class DQN2:
	...
	with graph2.as_default():
		...
		for i in range(len(trainable_variables_network)):
		    self.sess2.run(tf.assign(trainable_variables_target[i], trainable_variables_network[i]))
	...
	with graph2.as_default():
		...
		if check_save == 1:
		    ...
		    saver.restore(sess2, self.load_path + "/model.ckpt")
	...
	with graph2.as_default():
		Q_value = self.output.eval(feed_dict={self.x_image: [observation_stack], self.x_sensor: [state_stack]}, session=self.sess2)
	...
	with graph2.as_default():
		...
		if self.progress == 'Exploring':
		    ...
		    self.Replay_memory.append([observation, state, action, reward, next_observation, next_state, terminal])
		    ################################################### PER ############################################################
		    Q_batch = self.output_target.eval(feed_dict = {self.x_image: [next_observation],self.x_sensor:[next_state]}, session=self.sess2)
		    if terminal == True:
		        y = [reward]
		    else:
		        y = [reward + self.Gamma * np.max(Q_batch)]
		    TD_error = self.TD_error.eval(feed_dict = {self.action_target: [action], self.y_target: y, self.x_image: [observation], self.x_sensor:[state]}, session=self.sess2)[0]
    ...
    def return_action(self):
        self.action, Q_value = self.select_action(self.sess2, self.observation_stack, self.state_stack)
    ...
    def print_information(self):
        print('[Link2-Testing] step:'+str(self.step)+'/episode:'+str(self.episode)+'/path:'+self.path+'/score:'+ str(self.score))
    ...
    def new_environment(self):              
        ...
        # ------------------------------
        np.savetxt('/home/sdcnlab025/ROS_test/multi_qcar/src/tf_pkg/scripts/two_qcar_link_test_results/qcar2_reward.txt', self.reward_array, delimiter=',')     
    ...
    def end_func(self):
        '''plt.savefig('/home/sdcnlab025/ROS_test/multi_qcar/src/tf_pkg/scripts/two_qcar_link_test_results/qcar2_reward.png')
        plt.show()'''
