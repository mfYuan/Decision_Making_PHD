...
class envmodel1():
    ...
    def getreward(self):
        ...
        reward = reward - 0.005 * \
            (abs(self.w_last - self.cmd[1]) + abs(self.v_last - self.cmd[0]))
    ...
    def get_env(self):
        ...
        xp = dx*math.cos(self.robotstate1[4]) + \
            dy*math.sin(self.robotstate1[4])
        yp = -dx*math.sin(self.robotstate1[4]) + \
            dy*math.cos(self.robotstate1[4])
        ...
        elif (self.robotstate1[0] < -0.94 and self.robotstate1[1] > 0.18 and self.path == '6'):  # -0.94
            self.other_side = True
            self.done_list = True  # 终止 (terminated due to episode completed)
            #print("Reached other side!")

        # If qcar1 reaches end of path 7 (-0.18, -0.94)
        elif (self.robotstate1[0] < -0.18 and self.robotstate1[1] < -0.94 and self.path == '7'):  # -0.94
            self.other_side = True
            self.done_list = True  # 终止 (terminated due to episode completed)
            #print("Reached other side!")

        # If qcar1 reaches end of path 8 (0.94, -0.18)
        elif (self.robotstate1[0] > 0.94 and self.robotstate1[1] < -0.18 and self.path == '8'):  # 0.94
            self.other_side = True
            self.done_list = True  # 终止 (terminated due to episode completed)
            #print("Reached other side!")
    ...
    def step(self, cmd=[0.0, 0.0]):
        ...
        self.d = math.sqrt((self.robotstate1[0] - self.robotstate2[0])**2 + (
            self.robotstate1[1] - self.robotstate2[1])**2)
        #self.v_last = cmd[0]
        #self.w_last = cmd[1]
