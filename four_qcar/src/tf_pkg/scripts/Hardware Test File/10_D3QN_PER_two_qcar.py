...
from gazebo_env_D3QN_PER_two_qcar import envmodel
...
qcar2_array = np.array([0, 1, 1.5, 2, 2.5, 3])
#A = [0.18, -1.16]
#B = [-1.16, -0.18]
...
class DQN:
	...
	self.load_path = '/home/sdcnlab025/ROS_test/multi_qcar/src/tf_pkg/scripts/saved_networks/10_D3QN_PER_two_qcar_2_2021-02-09/400'
    ...
    def init_sess(self):
        ...
        # ------------------------------
        os.makedirs('saved_networks/' + '10_D3QN_PER_two_qcar'+ '_'+ self.Number + '_' + self.date_time)
        # ------------------------------
	...
	number = str(self.episode) 
        # ------------------------------
        save_path = self.saver.save(self.sess, 'saved_networks/' + '10_D3QN_PER_two_qcar'+ '_'+ self.Number + '_' + self.date_time + "/" + number + "/model.ckpt")
        # ------------------------------
	...
	v_qcar2 = np.random.choice(qcar2_array)
        ...
        while True:
            ...
            cmd       = [0.0, 0.0]
            v_cmd     = qcar1_dict[action_in][0]
            omiga_cmd = env.loop()#qcar1_dict[action_in][1]#env.self.calculateTwistCommand()#qcar1_dict[action_in][1]
            cmd[0]=v_cmd
            cmd[1]=omiga_cmd
	    time.sleep(0.05)
            env.step(cmd)
		...
		self.save_model()                
		print('Finished!!')
		plt.savefig('saved_networks/' + '10_D3QN_PER_two_qcar'+ '_'+ self.Number + '_' + self.date_time + '/reward.png')
		plt.show()
                break
		...
		if self.episode % 50 == 0:
		    self.save_model()
                ...
                step_for_newenv = 0
                # Print informations
                print('step:'+str(self.step)+'/episode:'+str(self.episode)+'/progress:'+self.progress+'/path:'+self.path+'/target:'+str(v_qcar2)+'/epsilon:'+str(self.Epsilon)+'/score:'+ str(self.score))

		plt.scatter(self.episode, self.score, c='r', s=((v_qcar2+1)*50))
		plt.xlabel("Episode")
		plt.ylabel("Reward")
		plt.xlim(-1, ((self.episode/100 + 1)*100))
		plt.ylim(-6, 3)
		plt.pause(0.05)
                ...
                if self.progress == 'Training':
                    ...
                    # ------------------------------
                    np.savetxt('10_D3QN_PER_two_qcar_reward.txt', reward_array, delimiter=',')                  
                    # ------------------------------
		...
		#randposition1 = -4.0 * np.random.rand() - 1.0		
		#randposition2 = -4.0 * np.random.rand() - 1.0
                    #if math.sqrt((randposition[0][0]-randposition[1][0])**2+(randposition[0][1]-randposition[1][1])**2) > 6.0:
                        #break

		# choose path in [5, 8]
		self.path = str(np.random.choice([5,8]))
		env.reset_env(path=self.path)
                env_info = env.get_env()

		v_qcar2 = np.random.choice(qcar2_array)
		#print("qcar2's speed is", v_qcar2)
                self.observation_stack, self.observation_set, self.state_stack, self.state_set = self.input_initialization(env_info)
