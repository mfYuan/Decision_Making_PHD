...
from gazebo_env_two_qcar_link1_hardwaretest import envmodel1
...
keras.backend.tensorflow_backend.set_session(tf.Session(graph=graph1, config=config1))
...
class DQN1:
	...
	self.load_path = '/home/sdcnlab025/ROS_test/multi_qcar/src/tf_pkg/scripts/saved_networks/two_qcar_links_2_2021-02-13/qcar1/4950'
    ...
    def update_parameters(self, Num_start_training, Num_training, Num_test, learning_rate, Gamma, Epsilon):
	...
	with graph1.as_default():
		...
		for i in range(len(trainable_variables_network)):
		    self.sess1.run(tf.assign(trainable_variables_target[i], trainable_variables_network[i]))
	...
	with graph1.as_default():
		Q_value = self.output.eval(feed_dict={self.x_image: [observation_stack], self.x_sensor: [state_stack]}, session=self.sess1)
		action = np.zeros([self.Num_action])
		#action = np.random.rand(1, self.Num_action)        
	...
	with graph1.as_default():
		...
		if self.progress == 'Exploring':
		    ...
		    self.Replay_memory.append([observation, state, action, reward, next_observation, next_state, terminal])
		    ################################################### PER ############################################################
		    Q_batch = self.output_target.eval(feed_dict = {self.x_image: [next_observation],self.x_sensor:[next_state]}, session=self.sess1)
		    if terminal == True:
		        y = [reward]
		    else:
		        y = [reward + self.Gamma * np.max(Q_batch)]
		    TD_error = self.TD_error.eval(feed_dict = {self.action_target: [action], self.y_target: y, self.x_image: [observation], self.x_sensor:[state]}, session=self.sess1)[0]
    ...
    def return_action(self):
        self.action, Q_value = self.select_action(self.sess1, self.observation_stack, self.state_stack)
	#print(Q_value)
    ...
    def update_information(self):
        ...
        # Experience Replay
        self.Experience_Replay(self.observation_stack, self.state_stack, self.action, reward, self.next_observation_stack, self.next_state_stack, terminal)
    ...
    def print_information(self):
        print('[Link1-Testing] step:'+str(self.step)+'/episode:'+str(self.episode)+'/path:'+self.path+'/score:'+ str(self.score))
    ...
    def new_environment(self):       
        ...
        # ------------------------------
        np.savetxt('/home/sdcnlab025/ROS_test/multi_qcar/src/tf_pkg/scripts/two_qcar_link_test_results/qcar1_reward.txt', self.reward_array, delimiter=',')     
    ...
    def end_func(self):
        plt.savefig('/home/sdcnlab025/ROS_test/multi_qcar/src/tf_pkg/scripts/two_qcar_link_test_results/qcar1_reward.png')
        plt.show()
