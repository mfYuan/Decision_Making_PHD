...
from gazebo_env_D3QN_PER_two_qcar_test import envmodel
...
#qcar2_array = np.array([0, 0.4, 0.8, 1, 1.4, 1.8, 2, 2.5, 3, 3.5, 4])
qcar2_array = np.array([0.0, 2.0])
...
class DQN:
	...
	self.load_path = '/home/sdcnlab025/ROS_test/multi_qcar/src/tf_pkg/scripts/saved_networks/10_D3QN_PER_two_qcar_2_2021-02-09/465'
	...
	#print("qcar2's speed is", v_qcar2)
	    ...
	    #color = str(v_qcar2/3.0)
	    #plt.scatter(self.episode, self.score, s = 100, c = color)
		...
		if v_qcar2 == 2.0:
		    ...
		    plt.scatter(self.episode, self.score, c='b', s=((v_qcar2+1)*50))
		plt.xlabel("Episode")
		plt.ylabel("Reward")
		...
		'''x = np.arange(self.MAXEPISODES)
		y = reward_array
		#c = np.array(v_qcar2_list)'''

		'''for i in range(self.MAXEPISODES):
		    if c[i] == 1.0:
			plt.plot(x[i], y[i], 'bo', label='1.0')
		    elif c[i] == 2.0:
			plt.plot(x[i], y[i], 'ro', label='2.0')
		    elif c[i] == 3.0:
			plt.plot(x[i], y[i], 'go', label='3.0')'''
		
		'''color = [str(item/4.0) for item in v_qcar2_list]

		plt.scatter(x, y, s = 250, c = color, cmap='coolwarm')
		plt.xlabel("Episode")
		plt.ylabel("Reward")
		#plt.legend()
		plt.show()
