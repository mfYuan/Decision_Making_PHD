...
from gazebo_env_car_following import envmodel
...
class DQN:
    def __init__(self):
        ...
        # 第二次训练
        #self.load_path = '/home/dell/DRL_Path_Planning/src/tf_pkg/scripts/saved_networks/10_D3QN_PER_image_add_sensor_obstacle_world_30m_2020-10-22'
        ...
        # Define desired distance between qcar1 and qcar2
        self.d_ego = 2.0
    ...
    def init_sess(self):
        ...
        os.makedirs('saved_networks/' + '10_D3QN_PER_car_following' + '_' + self.date_time)
    ...
    def select_action(self, progress, sess, state_stack, Epsilon):
        if progress == "Observing":
            ...
            if random.random() < Epsilon:
                ...
                # 否则，动作是具有最大Q值的动作
                Q_value = self.output.eval(feed_dict={self.x_sensor: [state_stack]})
                action = np.zeros([self.Num_action])
    ...
    def train(self, minibatch, w_batch, batch_index):
        ...
        # Selecting actions
        Q_network = self.output.eval(feed_dict = {self.x_sensor: state_next_batch})
        ...
        # Evaluation
        Q_target = self.output_target.eval(feed_dict = {self.x_sensor: state_next_batch})
    ...
    def Experience_Replay(self, state, action, reward, next_state, terminal):
		...
		if self.progress == 'Exploring':
			...
			self.Replay_memory.append([state, action, reward, next_state, terminal])
			# ################################################## PER ############################################################
			Q_batch = self.output_target.eval(feed_dict = {self.x_sensor:[next_state]})
			...
			TD_error = self.TD_error.eval(feed_dict = {self.action_target: [action], self.y_target: y, self.x_sensor:[state]})[0]
    ...
    def save_model(self):
        save_path = self.saver.save(self.sess, 'saved_networks/' + '10_D3QN_PER_car_following' + '_' + self.date_time + "/model.ckpt")
    ...
    def main(self):
        ...
        '''while(True):
            ...
            if math.sqrt((randposition[0][0]-randposition[1][0])**2+(randposition[0][1]-randposition[1][1])**2) > 20.0: ##### default 20.0
                break
	'''
        ...
        while True:
            ...
            action, Q_value = self.select_action(self.progress, self.sess, self.state_stack, self.Epsilon)
            ...
            self.next_state_stack, self.state_set = self.resize_input(env_info, self.state_set)  # 调整输入信息
            terminal = env_info[-2]  # 获取terminal
            reward = env_info[-1]  # 获取reward

            # Experience Replay
            self.Experience_Replay(self.state_stack, action, reward, self.next_state_stack, terminal)
            ...
            # self.progress == 'Finished' or self.episode==self.MAXEPISODES:
            ...
            if terminal == True:
                ...
                if self.progress == 'Training' and self.episode % 10 == 0:#self.progress == 'Training':
                    self.save_model()
                    reward_list.append(self.score)
                    reward_array = np.array(reward_list)
                    np.savetxt('10_D3QN_PER_car_following_reward.txt', reward_array, delimiter=',')
