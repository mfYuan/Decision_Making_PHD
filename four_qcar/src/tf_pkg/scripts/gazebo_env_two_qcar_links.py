...
class envmodel():
	...
	self.pub3 = rospy.Publisher('/' + self.agentrobot3 + '/ackermann_cmd', AckermannDriveStamped, queue_size=10)
    ...
    def reset_env(self, spawn_qcar2='1', spawn_qcar3='1'):
	'''self.path = path
        if path == '5':
        ...
        if spawn_qcar2 == '1':
            self.start2 = [-1.16, -0.18, 0.0]#-1.16

        else:
            self.start2 = [-100, -100, 0.0]

        if spawn_qcar3 == '1':
            self.start3 = [1.16, 0.18, np.pi]#1.16
