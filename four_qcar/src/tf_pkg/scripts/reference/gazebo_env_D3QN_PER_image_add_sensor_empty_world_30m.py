...
class envmodel():
    def __init__(self):
        ...
        self.subimage = rospy.Subscriber('/' + self.agentrobot +'/csi_front/image_raw', Image, self.image_callback)
        # 接收agent robot的激光雷达信息
        self.subLaser = rospy.Subscriber('/' + self.agentrobot +'/lidar', LaserScan, self.laser_states_callback)
        self.A=rospy.Subscriber('/qcar/rear_pose', PoseStamped, self.pose_cb, queue_size=1)
        self.B=rospy.Subscriber('/final_waypoints', Lane, self.lane_cb, queue_size=1)
        ...
        #self.loop()
	'''
	# 发布控制指令给agent robot (edited Nov 24 2020)
	
	self.pubLeftFront = rospy.Publisher('/left_front_axle_ctrlr/command', Float64, queue_size=10)
	self.pubRightFront = rospy.Publisher('/right_front_axle_ctrlr/command', Float64, queue_size=10)
	self.pubLeftRear = rospy.Publisher('/left_rear_axle_ctrlr/command', Float64, queue_size=10)
	self.pubRightRear = rospy.Publisher('/right_rear_axle_ctrlr/command', Float64, queue_size=10)
	self.pubLeftSteer = rospy.Publisher('/left_steering_ctrlr/command', Float64, queue_size=10)
	self.pubRightSteer = rospy.Publisher('/right_steering_ctrlr/command', Float64, queue_size=10)
	'''

	# Publishers (edited Nov 30 2020)
	

	#rate = rospy.Rate(10) # iterate at 10 Hz
	
	#self.cmd_vel    = AckermannDriveStamped()	
	#self.pub.publish(self.cmd_vel)

        # self.pub = rospy.Publisher('/left_front_axle_ctrlr/command', Twist, queue_size=10)
    ...
    def calculateTwistCommand(self):
        ...
        k =1
        ld = k * self.robotstate[5]
        print("value of vx is", self.robotstate[2],"value of vx is", self.robotstate[5], "value of ld is", ld)
    ...
    def reset_env(self, start=[16.0, 0.0], goal=[10.0, 10.0]):
        self.sp = start
        self.gp = goal
        # 初始点到目标点的距离
    ...
    # Publish velocity and steering cmd (edited Dec 3 2020)	
	...
	# call publish function repeatedly
	#cmd_publish(cmd_vel)	
	#schedule.every(0.1).seconds.do(cmd_publish, v=cmd_vel)

	'''
	cmd_vel = cmd[0]
	cmd_ang = cmd[1]	
	
	# Publish velocity cmd (edited Nov 24 2020)
	self.pubLeftFront.publish(cmd_vel)s
	self.pubRightFront.publish(cmd_vel)
	self.pubLeftRear.publish(cmd_vel)
	self.pubRightRear.publish(cmd_vel)

	# Publish steering cmd (edited Nov 24 2020)
	self.pubLeftSteer.publish(cmd_ang)
	self.pubRightSteer.publish(cmd_ang)
