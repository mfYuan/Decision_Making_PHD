...
class envmodel():
    ...
    def reset_env(self, start=[0.0, 0.0], goal=[10.0, 10.0]):
        ...
        #获取障碍的随机初始位置 (Get random initial position of obstacle)
        #障碍物为边长为1m的正方体 (The obstacle is a cube with a side length of 1 m)
        while(True):
            self.obs_pos = self.random_square(MAXENVSIZE/2)
            flag = True  # 用于判断起点和终点是否在障碍物的范围内 (Used to judge whether the start and end points are within the range of obstacles)
            for i in range(self.num_obs):
                # 如果起点在障碍物周围5.0m的范围内则需要重新生成障碍物 (If the starting point is within 5.0m of the obstacle, the obstacle needs to be regenerated)
                if math.sqrt((self.sp[0]-self.obs_pos[i][0])**2+(self.sp[1]-self.obs_pos[i][1])**2) < 5.0:
                    flag = False
                # 如果终点在障碍物周围5.0m的范围内则需要重新生成障碍物 (If the end point is within 5.0m of the obstacle, the obstacle needs to be regenerated)
                if math.sqrt((self.gp[0]-self.obs_pos[i][0])**2+(self.gp[1]-self.obs_pos[i][1])**2) < 5.0:
                    flag = False
                # 如果两个障碍相隔太近则需要重新生成障碍物 (If the two obstacles are too close, the obstacle needs to be regenerated)
