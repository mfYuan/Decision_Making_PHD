...
class DQN:
    ...
    def xavier_initializer(self, shape):
        ...
        return v1.random_uniform(shape, minval=-bound, maxval=bound)
    ...
    def network(self):
        ...
        # Input------image
        self.x_image = v1.placeholder(dtype=v1.float32, shape=[None, self.img_size, self.img_size, self.Num_stackFrame])
        self.x_normalize = (self.x_image - (255.0/2)) / (255.0/2)  # 归一化处理
        
        # Input------sensor
        self.x_sensor = v1.placeholder(dtype=v1.float32, shape=[None, self.Num_stackFrame, self.Num_dataSize])
        self.x_unstack = tf.unstack(self.x_sensor, axis=1)

        with v1.variable_scope('network'):
            ...
            # LSTM cell
            cell =v1.nn.rnn.BasicLSTMCell(num_units=self.Num_cellState)  
            #cell = v1.nn.rnn.BasicLSTMCell(num_units=self.Num_cellState) 
        ...
        with v1.variable_scope('target'):
            ...
            # LSTM cell
            cell_target = v1.contrib.rnn.BasicLSTMCell(num_units = self.Num_cellState)            
    ...
    def loss_and_train(self):
        ...
        action_target = v1.placeholder(dtype=v1.float32, shape=[None, self.Num_action])

        y_target = v1.placeholder(dtype=v1.float32, shape=[None])
        # 这里的 y_target 就是 target Q 值

        y_prediction = tf.reduce_sum(tf.multiply(self.output, action_target), reduction_indices=1)

        # ################################################## PER ############################################################
        w_is = v1.placeholder(dtype=v1.float32, shape = [None])
    ...
    def main(self):
        ...
        while True:
            ...
            if terminal == True:
                ...
                if self.progress == 'Training' and self.episode % 10 == 0:
