...
class DQN:
	...
	four = False
	    ...
	    if terminal_1:
		one= True
	    if terminal_2  or self.spawn_qcar2 == 0:
		two= True
	    if terminal_3  or self.spawn_qcar3 == 0:
		three= True
	    if terminal_4 or self.spawn_qcar4 == 0:
		four= True

            # Reset environment
            if self.step_for_newenv == self.MAXSTEPS or (one and two and three and four):
                # stop all vehicles
                env.stop_all_cars()
                time.sleep(0.1)
		...
		four = False
                ...
                self.spawn_qcar2 = str(input('Spawn Qcar2(0,1): '))
                self.spawn_qcar3 = str(input('Spawn Qcar3(0,1): '))
                self.spawn_qcar4 = str(input('Spawn Qcar4(0,1): '))

                # Choose level - 1 or 2
                self.level_qcar1 = str(input('Qcar1 select level(1,2,3): '))

                self.level_qcar2 = str(input('Qcar2 select level(1,2): '))
                self.level_qcar3 = str(input('Qcar3 select level(1,2): '))
                self.level_qcar4 = str(input('Qcar4 select level(1,2): '))


                link1.update_model(self.level_qcar1)
                ...
                qcar1_path = str(input('select path(10,5,4): '))
