...
class envmodel3():
    def __init__(self):
        ...
        self.qcar_dict = {0: (0.0, 0.0), 1: (0.0, thate_dot), 2: (0.0, -thate_dot), 
                          3: (nom_acc, 0.0), 4: (nom_acc, thate_dot), 5:(nom_acc, -thate_dot), 
                          6:(-nom_acc, 0.0), 7:(-nom_acc, thate_dot), 8:(-nom_acc, -thate_dot)}
        # self.qcar_dict = {0: (0.0, 0.0), 1: (0.0, 0.5), 2: (0.0, -0.5), 3: (0.25/k, 0.0), 4: (0.25/k, 0.5), 5:(0.25/k, -0.5), 6:(-0.25/k, 0.0), 7:(-0.25/k, 0.5), 8:(-0.25/k, -0.5)}
    ...
    def speed(self, pos, action, params):
        ...
        if delta_t > 1.0:
            delta_t = 0.1
        speed = self.robotstate3[2] + self.qcar_dict[action][0]*delta_t
        kp = 2
        delta_v = kp*(speed - self.robotstate3[2]) #error term
        self.previous = t1
        if speed >0.2:
            speed = 0.2
        elif speed < 0:
