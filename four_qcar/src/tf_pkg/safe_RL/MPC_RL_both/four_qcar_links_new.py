...
class DQN:
    ...
    def main(self):
        ...
        Pos_his = np.zeros((1, link1.env.max_step, 11, link1.env.num_cars))
        
        creat_folder = input('new folder name: ')
        path = '/home/sdcnlab025/ROS_test/four_qcar/src/tf_pkg/safe_RL/MPC_RL_both/Images/Recorded/'+str(creat_folder)
        # Check whether the specified path exists or not
        isExist = os.path.exists(path)
        path_ls = []
        if not isExist:
            name_ls = ['/motion', '/graph', '/others', '/level']
            for i in name_ls:
