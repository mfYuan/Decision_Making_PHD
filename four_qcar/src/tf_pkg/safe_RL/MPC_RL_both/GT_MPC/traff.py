...
def initial():
    ...
    traffic = Bunch( 
        ...
        previous_steering = np.empty(shape=[1, 0])
        )
    return traffic

def update(traffic, x_car, y_car, orientation_car, v_car, AV_flag, Final_x, Final_y, path_id, Currend_index, Driver_level, previous_steering):
    ...
    traffic.previous_steering = np.append(traffic.previous_steering, [[previous_steering]], axis=1)
    return traffic
