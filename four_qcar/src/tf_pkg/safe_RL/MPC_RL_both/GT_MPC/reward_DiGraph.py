...
def reward(X_reward, car_id, action_id, params, dist_id, Level_ratio, L_matrix_all):
    ...
    #On road status TO DO------
    background = params.inter
    bound3 = np.array(background[str(3)])  
    bound4 = np.array(background[str(4)])  
    bound5 = np.array(background[str(5)])  
    bound6 = np.array(background[str(6)])  
    
    #Distance to lane center
    Deviation = 0
    Deviation_Penalty = -1e5
    closest_point_in_path = params.waypoints[str(int(X_reward[7, car_id]))][int(X_reward[8, car_id])]
   
    Dis2center  = math.sqrt((X_reward[0, car_id] - closest_point_in_path[0])**2+
                            (X_reward[1, car_id] - closest_point_in_path[1])**2)
    
    Deviation = Deviation + Deviation_Penalty * Dis2center
    ...
    # Local Reward
    R_l = (Colli + Safe + 0*conflict + Complete + Speed + Deviation)
    # if X_reward[4,car_id] == 1 and not(abs(X_reward[6,car_id]-(X_reward[1,car_id])) <= w_lane/2):
