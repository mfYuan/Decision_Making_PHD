...
def motion_update(X_old, car_id, action_id, t_step, params, L_matrix_all):
    ...
    thate_dot = params.thata_dot
    nom_acc = params.max_acc
    qcar_dict = {0: (0.0, 0.0), 1: (0.0, thate_dot), 2: (0.0, -thate_dot), 3: (nom_acc, 0.0), 4: (nom_acc, thate_dot), 5:(nom_acc, -thate_dot), 6:(-nom_acc, 0.0), 7:(-nom_acc, thate_dot), 8:(-nom_acc, -thate_dot)}
    
    acceleration = qcar_dict[action_id][0]
    thata_dot = qcar_dict[action_id][1]#calculateTwistCommand(X_old, car_id, params)
    thata = X_old[10, car_id] + thata_dot * t_step
    ...
    #speed
    X_new[3,car_id] = X_new[3,car_id]+acceleration*t_step
    if X_new[3,car_id]>params.v_max:
        X_new[3,car_id]=params.v_max
    elif X_new[3,car_id]<params.v_min:
