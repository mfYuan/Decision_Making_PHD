...
import sys
sys.path.append('/home/sdcnlab025/ROS_test/four_qcar/src/tf_pkg/safe_RL/MPC_RL_both/GT_MPC')
import get_params
...
class envmodel1():
    ...
    #         '/' + self.agentrobot1 + '/csi_front/image_raw', Image, self.image_callback)
        ...
        thate_dot = self.params.thata_dot
        nom_acc = self.params.max_acc
        self.qcar_dict = {0: (0.0, 0.0), 1: (0.0, thate_dot), 2: (0.0, -thate_dot), 3: (nom_acc, 0.0), 4: (nom_acc, thate_dot), 5:(nom_acc, -thate_dot), 6:(-nom_acc, 0.0), 7:(-nom_acc, thate_dot), 8:(-nom_acc, -thate_dot)}
            # Figure size
    ...
    def get_position(self, get_path, Driver_level):
        ...
        vehicle_states = np.block ([[self.traffic.x], [self.traffic.y], [self.traffic.orientation], 
                               ...
                               [self.traffic.Final_x], [self.traffic.Final_y], [self.traffic.path_id], [self.traffic.Currend_index], [self.traffic.Driver_level],[self.traffic.previous_steering]])
    ...
    def vehicle_position(self, params, traffic, AV_cars, get_path, Driver_level):       
        ...
        for id in range(0, self.num_cars):
            ...
            traffic = traff.update(traffic, x_car, y_car, orientation_car, v_car, AV_flag, 
                                   float(init_index), float(final_index), float(path_id), 
                                   float(Current_index), float(Dr_level), float(previous_steering))
    ...
    def veh_pos_realtime(self, X_old):       
        for id in range(0, self.num_cars):
            ...
            X_old[10][id] = self.previous_angle
        self.X_old = X_old
    ...
    def speed(self, pos, action, params):
        ...
        if delta_t > 1.0:
            delta_t = 0.1
        speed = self.robotstate1[2] + self.qcar_dict[action][0]*delta_t
        kp = 2
        ...
        # self.accel_current = self.qcar_dict[action][0]
         
        self.cmd[0] = speed*2.5# - delta_v
        if self.cmd[0] < 0:
            self.cmd[0]=0
            
        # self.cmd[1] = self.SteeringCommand(pos, car_id, params)#self.loop()
        self.cmd[1] = self.previous_angle + self.qcar_dict[action][1]*delta_t#self.loop()
        if self.cmd[1]>0.5:
            self.cmd[1] = 0.5
        print('delta_t', delta_t)
        self.previous_angle = self.cmd[1]
        ...
        cmd_vel.drive.steering_angle = self.cmd[1]
        #if self.cmd[0]:
        cmd_vel.drive.acceleration = self.qcar_dict[action][0]
        cmd_vel.drive.steering_angle_velocity =  self.qcar_dict[action][1]
        self.pub.publish(cmd_vel)
