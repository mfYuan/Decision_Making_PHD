...
class vehicle_pose_and_velocity_updater:
	def __init__(self):
		...
		self.rear_pose_pub = rospy.Publisher('/qcar2/rear_pose', PoseStamped, queue_size = 1)
		self.center_pose_pub = rospy.Publisher('/qcar2/center_pose', PoseStamped, queue_size = 1)
		self.front_pose_pub = rospy.Publisher('/qcar2/front_pose', PoseStamped, queue_size = 1)
		self.vel_pub = rospy.Publisher('/qcar2/velocity', TwistStamped, queue_size = 1)
		rospy.Subscriber('/gazebo/model_states', ModelStates, self.model_cb, queue_size = 1)
		rospy.spin()

	def model_cb(self,data):
		try:
			vehicle_model_index = data.name.index("qcar2")
		except:
			return
		...
		rear_x = center_x - cos(yaw) * 0.128
		rear_y = center_y - sin(yaw) * 0.128
		...
		center_x = vehicle_position.position.x
		center_y = vehicle_position.position.y
		front_x = center_x + cos(yaw) * 0.128
		front_y = center_y + sin(yaw) * 0.128
