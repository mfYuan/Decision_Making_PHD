...
class vehicle_pose_and_velocity_updater:
	def __init__(self):
		...
		self.rear_pose_pub = rospy.Publisher('/qcar8/rear_pose', PoseStamped, queue_size = 1)
		self.center_pose_pub = rospy.Publisher('/qcar8/center_pose', PoseStamped, queue_size = 1)
		self.front_pose_pub = rospy.Publisher('/qcar8/front_pose', PoseStamped, queue_size = 1)
		self.vel_pub = rospy.Publisher('/qcar8/velocity', TwistStamped, queue_size = 1)
		rospy.Subscriber('/gazebo/model_states', ModelStates, self.model_cb, queue_size = 1)
		rospy.spin()

	def model_cb(self,data):
		try:
			vehicle_model_index = data.name.index("qcar8")
		except:
			return
