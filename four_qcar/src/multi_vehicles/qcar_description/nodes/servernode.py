...
class ServerNode(object):
	def __init__(self):
		...
		self.num_measurements = 7200
		self.scans = np.zeros((self.num_measurements,1), dtype= np.float64)
		self.pose_num = 7

		self.myServer = BasicStream('tcpip://localhost:18999', agent='s', send_buffer_size = self.pose_num * 8, recv_buffer_size=self.num_measurements * 8)
		self.prev_con = False

		while True: #We have a problem here
			if not myServer.connected:
				myServer.checkConnection()

			if myServer.connected and not self.prev_con:
				print('Connection to Client was successful.')
			self.prev_con = myServer.connected

			if myServer.connected:
				starTime = time.time()
				lidar_data, bytes_received = myServer.receive(self.scans)
				if bytes_received == 0:
